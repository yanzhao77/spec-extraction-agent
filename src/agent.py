...
# Engineering Specification Extraction Agent MVP v2.3 (Final)
# Enhanced error handling and self-repair capabilities.
...
class LLMClient:
    ...
    def call(self, system_prompt: str, user_prompt: str, timeout: int = 20) -> Optional[str]:
        if not self.client:
            ...
            # This mock response is intentionally malformed to test the REPAIR state
            if "防火墙" in user_prompt:
                return '{{"applicable_object": "防火墙", "constraint_content": "耐火极限", "value": 4.0, "operator": ">="}}' # Missing unit and not a list
            return '[]'
...
class ExtractionAgentFinal:
    def __init__(self, document_path: str):
        ...
        logger.info(f"Agent v2.3 initialized for document: {document_path}")
    ...
    def _analyze_structure(self):
        ...
        for i, section in enumerate(sections):
            if len(section.strip()) > 50: # Filter out small/empty sections
    ...
    def _validate(self):
        newly_failed = []
        # Process results from EXTRACTION or REPAIR states
    ...
    def _finalize(self):
        for item in self.validated_items:
            ...
            item['extraction_metadata'] = {
                ...
                "confidence_score": 0.98 # Higher confidence after validation
            }
        self.final_output = json.dumps(self.validated_items, indent=2, ensure_ascii=False)
    ...
    def _handle_error(self):
        ...
        self.final_output = json.dumps({"error": self.error_message}, indent=2)
        self._set_state(AgentState.DONE)
...
if __name__ == "__main__":
    agent = ExtractionAgentFinal(document_path="GB50016_2014_sample.txt")
    final_json_output = agent.run()
    print("\n" + "="*50 + " FINAL OUTPUT " + "="*50)
    print(final_json_output)
    print("\n" + "="*114)
    print("Full execution trace is available in 'agent_mvp_final.log'")
